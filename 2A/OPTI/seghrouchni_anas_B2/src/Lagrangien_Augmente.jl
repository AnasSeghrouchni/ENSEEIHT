...
function Lagrangien_Augmente(algo,fonc::Function,contrainte::Function,gradfonc::Function,
  ...
  eta0 = eta / (mu0^alpha)
  etak = eta0
  Non_arret = !((norm(gradfonc(x0) + lambdak' * grad_contrainte(x0))) <= max(tol*norm(gradfonc(x0) + lambdak' * grad_contrainte(x0)), tol)) && iter < itermax 
  lagrangien_a(x) = fonc(x) + lambdak' * contrainte(x) + (muk/2) * norm(contrainte(x))^2
  grad_la(x) = gradfonc(x) + lambdak' * grad_contrainte(x) + muk * contrainte(x) * grad_contrainte(x)
  hess_la(x) = hessfonc(x) + lambdak' * hess_contrainte(x) + muk * (grad_contrainte(x)*grad_contrainte(x)' + contrainte(x) * hess_contrainte(x))
  lagrangien(x) = fonc(x) + lambdak' * contrainte(x)
  grad_l(x) = gradfonc(x) + lambdak' * grad_contrainte(x)
  while Non_arret
    if algo == "newton"
      xkp,fxk,flagk,iterk = Algorithme_De_Newton(lagrangien_a, grad_la,hess_la, xk,[itermax,epsk,tol,tol])
    else
      xkp,fxk,flagk,iterk = Regions_De_Confiance(algo,lagrangien_a, grad_la,hess_la, xk,[10,0.5,2.00,0.25,0.75,2,100,epsk,1e-15,epsilon])
    end
    if norm(contrainte(xkp)) <= etak 
      lambdak = lambdak + muk * contrainte(xkp)
    ...
    crit1 = (norm(grad_la(xkp)) <= max(tol*norm(grad_la(x0)), tol))
    crit4 = iter + 1 <= itermax
    if (crit1) 
        flag = 0
    elseif ~(crit4)
            flag = 3 
    ...
    xk =xkp
    Non_arret = ~crit1 && crit4
  end      
  println(lambdaks)
  println(muks)
        return xk,fonc(xk),flag,iter, muks, lambdaks
end