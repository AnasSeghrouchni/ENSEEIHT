...
function Algorithme_De_Newton(f::Function,gradf::Function,hessf::Function,x0,options)
    ...
    if options == []
        ...
        while Non_arret 
            ...
            xmin = xmin + d_k
            nb_iters = nb_iters + 1
            crit1 = norm(gradf(xmin)) <= max(Tol_rel*norm(gradf(x0)), Tol_abs)
            crit2 = norm(d_k) <= epsilon*max(Tol_rel*norm(xmin-d_k), Tol_abs)
            crit3 = norm(f(xmin) - f(xmin - d_k)) <= epsilon*max(Tol_rel*abs(f(xmin - d_k)), Tol_abs)
