...
! calcul du premier pivot (celui de degre minimum)
       ...
       do while (next /= dim)
              do i = sideb, sifin
                     courant => g(perm(i))%suivant
                     do while (associated(courant))
                            if ( g(courant%indice)%indice /= -1 ) then
                                   next = next + 1
                                   perm(next) = courant%indice
                                   g(courant%indice)%indice = -1
                            end if
...
! variables locales
       integer :: degmin, min_int, j
       degmin = dim + 1
       do j = 1, dim
              min_int = g(j)%indice
              if (min_int /= -1) then 
                     if (min_int < degmin)  then 
                            pivot = j
                            degmin = min_int
                     end if
              end if   
