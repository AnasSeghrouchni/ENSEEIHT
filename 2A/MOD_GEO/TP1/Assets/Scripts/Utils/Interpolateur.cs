...
public class Interpolateur : MonoBehaviour
{
    ...
    (List<float>, List<float>) buildParametrisationDistance(int nbElem, float pas)
    {
        ...
        for(int i =1;i<nbElem;i++){
            d = (float)Mathf.Sqrt(Mathf.Pow(X[i]-X[i-1],2)+Mathf.Pow(Y[i]-Y[i-1],2));
            T.Add(T[i-1] + d);
        }
        // Construction des échantillons
        for(int i=0;i<T.Count-1;i++){
    ...
    (List<float>, List<float>) buildParametrisationRacineDistance(int nbElem, float pas)
    {
        ...
        float d = 0f;
        for(int i =1;i<nbElem;i++){
            d = (float)Mathf.Sqrt(Mathf.Sqrt(Mathf.Pow(X[i]-X[i-1],2)+Mathf.Pow(Y[i]-Y[i-1],2)));
            T.Add(T[i-1] + d);
        }
        // Construction des échantillons
        for(int i=0;i<T.Count-1;i++){
    ...
    {    
        ...
        for(int j = 1;j<X.Count;j++){
            for(int i=0;i<X.Count-j;i++){
                Xtemp[i] = ((T[i+j]-t)/(T[i+j]-T[i]))*Xtemp[i]+((t-T[i])/(T[i+j]-T[i]))*Xtemp[i+1];
                Ytemp[i] = ((T[i+j]-t)/(T[i+j]-T[i]))*Ytemp[i]+((t-T[i])/(T[i+j]-T[i]))*Ytemp[i+1];
            }
        }  
