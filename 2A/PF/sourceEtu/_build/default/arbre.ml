...
let rec retrait_arbre e (Noeud (b,lb)) =
   match e with
   |[]-> Noeud(false,lb)
   |t::q -> match recherche t lb with
            |None -> (Noeud (b,lb))
            |Some a -> Noeud (b, maj t (retrait_arbre q a) lb )



let arbre_test = List.fold_right ajout_arbre
[['a';'n';'a';'s']; ['b';'a';'s']; ['l';'a';'i';'t']; ['b';'a';'t']; ['l';'e']; ['d';'e'];
 ['l';'a';'i']; ['l';'a';'i';'d']; ['l';'e';'s']; ['l';'a';'r';'d'];
 ['l';'a']; ['l';'o';'n';'g']]
(Noeud (false,[])) 

let%test _= retrait_arbre ['a';'n';'a'; 's'] arbre_test = arbre_sujet
let%test _= retrait_arbre ['b';'o';'n'] arbre_test = arbre_test

let rec list_lex (Noeud (b,lb)) = 
   match lb with
   |[] -> if b then [[]] else []
   |(c,a)::q -> let liste = List.map (fun l -> c::l) (list_lex a) in
   if b then ([]::liste)@(list_lex (Noeud(false,q))) else liste@(list_lex (Noeud(false,q)))

let%test _= list_lex arbre_test = [['a'; 'n'; 'a'; 's']; ['b'; 'a'; 's']; ['b'; 'a'; 't']; ['d'; 'e'];
['l'; 'a']; ['l'; 'a'; 'i']; ['l'; 'a'; 'i'; 'd']; ['l'; 'a'; 'i'; 't'];
['l'; 'a'; 'r'; 'd']; ['l'; 'e']; ['l'; 'e'; 's']; ['l'; 'o'; 'n'; 'g']]
