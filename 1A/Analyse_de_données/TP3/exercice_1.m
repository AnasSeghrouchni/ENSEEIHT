...
%Fonction moyenne qui calcul la couleur moyenne d'une image

function X_moy=moyenne(image)
    im_reel=single(image);
    R=im_reel(:,:,1);
    V=im_reel(:,:,2);
    B=im_reel(:,:,3);
    norm = (im_reel./max(1,R+V+B));
    X_moy=[ mean(mean(norm(:,:,1)))' mean(mean(norm(:,:,2)))'];
end
