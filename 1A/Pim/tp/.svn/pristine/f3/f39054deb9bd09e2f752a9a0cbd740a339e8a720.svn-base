...
procedure Parenthesage is
    ...
    procedure Verifier_Parenthesage (Chaine: in String ; Correct : out Boolean ; Indice_Erreur : out Integer) is
        ...
        package Pile_Caractere is
         ...
         package Pile_Indice is
                new Piles (Capacite => (Chaine'Last)-(Chaine'First), T_Element => Integer);
        use Pile_Indice;
        use Pile_Caractere;

        Pile_Ouvrante : Pile_Caractere.T_Pile;
        Pile_Index : Pile_Indice.T_Pile;
        i : Integer;
    begin
        i:=Chaine'First;
        Correct:=True;
        for i in 1..(Chaine'Last) loop
            for j in 1..3 loop
                if Chaine(i)=Ouvrants(j) then
                    Pile_Caractere.Empiler(Pile_Ouvrante, Chaine(i));
                    Pile_Indice.Empiler(Pile_Index, i);
                else null;
                end if;
                end loop;
        end loop;
        while not(Est_Vide(Pile_Ouvrante)) and Correct and i<=Chaine'Last loop
            for j in 1..3 loop
                if Chaine(i)=Fermants(j) then
                    Indice_Erreur:=2;
                    Correct:=Pile_Caractere.Sommet(Pile_Ouvrante)=Ouvrants(j);
                    Depiler(Pile_Ouvrante);
                    Depiler(Pile_Index);
                    i:=i+1;
                else
                    null;
