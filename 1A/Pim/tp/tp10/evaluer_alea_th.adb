...
procedure Evaluer_Alea_TH is
	...
	is
        ...
        package TH_alea is new TH (T_Cle_t=>Integer, T_Donnee_t=>Integer,Capacite=>1000,Fh=> Ident);
        use TH_alea;
    ...
    begin
        ...
        for i in 1..Taille loop
            ...
            if Cle_Presente_t(S,Aleatoire) then
                Enregistrer_t(S,Aleatoire,La_Donnee_t(S,Aleatoire)+1);
            else
                Enregistrer_t(S,Aleatoire,1);
            end if;
            D:=La_Donnee_t(S,Aleatoire);
            if D>Max then
                Max:=D;
            elsif Aleatoire=Min_cle then
                Min:=Min+1;
            elsif D<=Min then
                Min:=D;
                Min_cle:=Aleatoire;
...
begin
    ...
    when Constraint_Error =>
        Put("Relancez le programme en entrant 2 entiers strictement supérieur à 1");
