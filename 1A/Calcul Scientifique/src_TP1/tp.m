...
for i = 1:k
  ...
  % Perte d'orthogonalite avec algorithme classique 2x
  Qcgss = cgs(cgs(A));
  po(3,i) = norm(eye(n)-Qcgss'*Qcgss);
  
  % Perte d'orthogonalite avec algorithme modifie 2x
  Qmgss = mgs(mgs(A));
  po(4,i) = norm(eye(n)-Qmgss'*Qmgss);
