...
% SUBSPACE ITERATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[D, V_2, n_ev, it, itv, flag ] = subspace_iter_v2(Sigma2, 16, 1, 8, 1e-8, 10000);
%[V_2, D] = eig(Sigma2);
...
R=reshape(individu_moyen_masque, [400,300]);
...
% Calcul des vecteurs/valeurs propres de la matrice Sigma2 :
%[VecteursP_S2_masque, ValeursP_masque] = eig(Sigma2_masque);
[ValeursP_masque, VecteursP_S2_masque, n_ev, it, itv, flag ] = subspace_iter_v2(Sigma2_masque, 16, 1, 8, 1e-8, 10000);


% Les vecteurs propres de Sigma (les eigenfaces) se deduisent de ceux de Sigma2 :
VecteursP_S1_masque = X_centre_masque'*VecteursP_S2_masque;

% Tri par ordre decroissant des valeurs propres de Sigma_barre :
[ValeursP_triees_masque, passage_masque] = sort(diag(ValeursP_masque), 'descend');

% Tri des eigenfaces dans le meme ordre 
% (on enleve la derniere eigenface, qui appartient au noyau de Sigma) :

VecteursP_S1_masque = VecteursP_S1_masque(:,passage_masque');
VecteursP_S1_masque = VecteursP_S1_masque(:, 1:n-1);

% Normalisation des eigenfaces :
VecteursP_S1_masque = normc(VecteursP_S1_masque);
% les eigenfaces avec masque
 W_masque = VecteursP_S1_masque;
...
% for k = 1:n-1,
% 	img_masque = reshape(W_masque(:,k), nb_lignes,nb_colonnes);
% 	subplot(nb_personnes_base, nb_postures_base,k+1);
% 	imagesc(img_masque); 
% 	hold on; 
% 	axis image; 
